drop database depitest;
create database if not exists depitest;
...
create user if not exists depitest identified by 'depitest';
grant all on depitest.* to 'depitest'@'localhost';
flush privileges;

call DOLT_COMMIT('-A', '-m', 'Initial commit', '--author', 'Mark Wutka <mark.wutka@vanderbilt.edu>');
