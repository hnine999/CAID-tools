...
describe('generate template model', () => {
    // N.B! These depend on the __mocks__/vscode.js heavily, esp. check the stat mock and 'provided-dir' name for dir!
    it('should generate a new model file in a dir at the provided dir', async () => {
        const modelUri = await generateNewModel(modelDirUri, 'provided-dir');
        expect((modelUri as any).endsWith(`test-util-dir${path.sep}provided-dir`)).toBeTruthy();
    });

    it('should generate a new model file in a dir next to the provided file', async () => {
        const modelUri = await generateNewModel(Uri.file(path.join(modelDirPath, 'file.txt')), 'provided-dir');
        expect((modelUri as any).endsWith(`test-util-dir${path.sep}provided-dir`)).toBeTruthy();
    });

    it('should generate the provided model in contents', async () => {
        const content = new Map<string, string>();

        content.set('nsp1', `GOALS nsp1
    goal G1
        summary "This is the top-goal"
`);
        content.set('nsp2', `GOALS nsp12
    goal G2
        summary "This is another goal"
`);
        const modelUri = await generateNewModel(modelDirUri, 'provided-dir', content);
        expect((modelUri as any).endsWith(`test-util-dir${path.sep}provided-dir`)).toBeTruthy();
    });
});