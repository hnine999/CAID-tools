	...
	def String setFeature(ALLNodes node, String attr, Object newValue)
	{
		...
		switch(attr)
		{
            ...
            case "status":
            {
                ...
                var className = node.eClass().getClassifierID();
                if (className == AssurancePackage.SOLUTION_NODE) {
                    var SolutionNode s = (node as SolutionNode)
                    s.eSet(s.eClass().getEStructuralFeature(AssurancePackage.SOLUTION_NODE__STATUS), x)
                } else {
                    var GoalNode g = (node as GoalNode)
                    g.eSet(g.eClass().getEStructuralFeature(AssurancePackage.SOLUTION_NODE__STATUS), x)
                }
            }
			default:
