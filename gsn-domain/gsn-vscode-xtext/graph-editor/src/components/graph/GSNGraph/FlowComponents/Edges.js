...
const pathStyles = (selected, searchString, nonReactive, isRef, isContext) => ({
    strokeWidth: selected ? 4 : 2,
    strokeOpacity: searchString ? 0.1 : isRef ? 0.6 : 1,
    stroke: selected ? COLORS.SELECTED(nonReactive) : COLORS.Edge,
    strokeDasharray: isRef ? 3 : isContext ? 6 : undefined,
});
...
function SolvedBy({ id, data, ...pathProps }) {
    ...
    const isRef = connInfo && isReference(connInfo.srcId, connInfo.dstId);
    const showRefMarker = isRef && showReferences;
...
function InContextOf({ id, data, ...pathProps }) {
    ...
    return (
        <>
            <path
                id={id}
                style={pathStyles(selected, searchString, nonReactive, isRef, true)}
                className="react-flow__edge-path"
                d={edgePath}
