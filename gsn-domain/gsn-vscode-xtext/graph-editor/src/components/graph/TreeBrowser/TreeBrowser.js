...
// Threshold [0, 1] for fuzzy search to be treated as a match.
// This search does not apply to info/summary search and only to name and label matches.
// https://www.npmjs.com/package/fast-fuzzy?activeTab=readme#examples
...
export default function TreeBrowser({ data, selectedNode, setSelectedNode, width }) {
    ...
    const searchRoots = useMemo(() => {
        ...
        data.forEach((node) => {
            function compare(values, simpleCompare) {
                ...
                for (const value of values) {
                    if (simpleCompare) {
                        ...
                        const fuzzyValue = fuzzy(compareString, value);

                        if (fuzzyValue > FUZZY_THRESHOLD) {
                            return true;
                        }
                    }
                }
            ...
            switch (searchCategory.id) {
                ...
                case SEARCH_CATEGORIES.info.id:
                    match = compare([node.summary || '', node.info || ''], true);
                    break;
