...
export default function AttributeForm({
    ...
    const [minReqOpts, parentId, nodeId, infoText] = useMemo(() => {
        ...
        if (nodeData.info) {
            ...
            infoText = isReadOnly || reviewMode ? 'No info added ..' : 'Add info text ..';
        }

        return [minReqOpts, parentId, nodeData.id, infoText];
    }, [nodeData, isReadOnly, reviewMode]);
        ...
        <Grid container spacing={1} style={{ padding: 10 }}>
            ...
            <WrapWithLabel>
                <Button variant="text" size="small" onClick={() => setTopMenuIndex(1)}>
                    {`${isReadOnly || reviewMode ? 'SHOW' : 'EDIT'} LABELS ...`}
                </Button>
            </WrapWithLabel>
        </Grid>
    );
