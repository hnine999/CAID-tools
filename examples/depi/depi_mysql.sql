...
create user if not exists depiadmin identified by 'depiadmin';
grant all on *.* to 'depiadmin'@'%';
flush privileges;

create user if not exists depi identified by 'depi';
grant all on depi.* to 'depi'@'%';
flush privileges;
